...
3. 阅读 `memory_data/memory/YYYY-MM-DD.md`（今天 + 昨天）获取近期上下文
4. **如果在主会话中**（与你的人类直接对话）：还要阅读 `memory_data/MEMORY.md`
...
**重要：** 记忆工作目录是 `/workspace/memory_data/`（不是 `/workspace/memory/`）

每次会话你都是全新启动。记忆文件是你的连续性保障。**记忆必须写入文件，不要"心理记忆"！**

### 目录结构

```
/workspace/memory_data/              # ⚠️ 记忆工作目录（固定路径）
├── dialog/                          # 📝 会话对话记录
│   ├── {session_id}_{2026-05-09}.jsonl    # 每条消息一行JSON
│   └── ...
│
├── memory/                          # 📋 每日记忆摘要（必须更新）
│   ├── 2026-05-11.md                     # 今天发生的原始记录
│   ├── 2026-05-10.md                     # 昨天的记录
│   └── ...
│
├── memory_qa/                       # ❓ 每日记忆QA对（索引副产物）
│   ├── 2026-05-11_qa.md
│   └── MEMORY_QA.md                     # 长期记忆QA
│
├── MEMORY.md                        # 🗂️ 长期记忆（精华汇总）
│
├── MEMORY_QA.md                     # 🔍 长期记忆QA索引
│
├── memory_index/                    # 🔎 向量索引（AI理解用）
│   └── embeddings.db
│
├── bm25_index/                      # 📊 BM25关键词索引（搜索用）
│
└── tool_result/                    # 🔧 工具结果存档
```
...
| 时机 | 操作 | 目标文件 |
|------|------|----------|
| 会话结束/重要决策 | 记录发生的事情、决策、结论 | `memory_data/memory/YYYY-MM-DD.md` |
| 学到教训/犯错误 | 记录避免重蹈覆覆的方法 | `memory_data/MEMORY.md` 或相关 Skills |
| 收到明确指示 | 记录格式偏好、风格要求 | `memory_data/memory/YYYY-MM-DD.md` |
| 每隔几天 | 整理提炼，更新长期记忆 | `memory_data/MEMORY.md` |
...
- 当有人说"记住这个" → 更新 `memory_data/memory/YYYY-MM-DD.md` 或相关文件
...
1. 阅读最近的 `memory_data/memory/YYYY-MM-DD.md` 文件
2. 识别值得长期保留的重要事件、教训或见解
3. 用提炼的内容更新 `memory_data/MEMORY.md`
